...
if (Test-Path -Path $FolderName -PathType Container) {   
    ...
    Get-ChildItem -Path "$OutputFolderPath\*GitHubBackupTool*Build at*.exe" -Recurse | Move-Item -Destination $FolderName
    Write-Host "Moved files $OutputFolderPath\*GitHubBackupTool*Build at*.exe to $FolderName"
} else {
...
Get-ChildItem -Path "$OutputFolderPath\*GitHubBackupTool*Build at*" -File | Where-Object CreationTime -lt (Get-Date).AddSeconds(-2) | Remove-Item -Force

# Get the file version for the last build of GitHubBackupTool.exe
$FileVersion = [System.Diagnostics.FileVersionInfo]::GetVersionInfo("$OutputFolderPath\GitHubBackupTool.exe").FileVersion

# Rename the file to include version and build time (keep original)
Get-ChildItem -Path "$OutputFolderPath\GitHubBackupTool.exe" | Where-Object {!$_.PSIsContainer -and $_.Extension -eq '.exe'} | ForEach-Object {
